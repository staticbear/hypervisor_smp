...
#define STACK64_addr		(HYPERVISOR_base + HYPERVISOR_size + STACK64_size * N_CORES) 	 

#define STACK64i_size		0x1000
#define STACK64i_addr		(STACK64_addr + STACK64i_size * N_CORES)              
...
#define VMXON_addr			(TSS_addr + TSS_size * N_CORES)

#define VMCS_size			0x1000
#define VMCS_addr			(VMXON_addr + VMXON_size * N_CORES)

#define MSR_BMP_size		0x1000
#define MSR_BMP_addr		(VMCS_addr + VMCS_size * N_CORES)
...
#define MSR_EX_STR_addr		(MSR_EX_LDR_addr + MSR_EX_LDR_size * N_CORES) /* VM-exit MSR-store address*/
...
#define EPT_PML4_addr		(MSR_EN_LDR_addr + MSR_EN_LDR_size * N_CORES)
...
#define	GUEST_REGS_addr		(EPT_PT_addr + EPT_PT_size)

#define	SERIAL_EVENT_size	8
#define	SERIAL_EVENT_addr	(GUEST_REGS_addr + GUEST_REGS_size * N_CORES)

#define DEBUG_EVENT_size	8
#define DEBUG_EVENT_addr	(SERIAL_EVENT_addr + SERIAL_EVENT_size)

#define SYSENTER_size		(8 * 4)
#define SYSENTER_addr		(DEBUG_EVENT_addr + DEBUG_EVENT_size)
